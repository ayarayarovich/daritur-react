...
export const Route = createFileRoute('/_private/_layout/tours_/$id')({
...
const baseFormScheme = z.object({
  ...
  type: z.string().refine(...requiredFieldRefine()),
  ...
  approveType: z.string().refine(...requiredFieldRefine()),
...
function RouteComponent() {
  ...
  const form = useForm<z.infer<typeof formScheme>>({
    ...
    defaultValues: async () => {
      const tour = await Query.client.fetchQuery(Queries.tours.detail({ id: ctx.params.id }))
      return {
        tourId: tour.id,
        name: tour.name,
        ...
        priceTransferAdult: tour.priceTransferAdult,
        priceTransferChild: tour.priceTransferChild,
        priceParticipateAdult: tour.priceParticipateAdult,
        priceParticipateChild: tour.priceParticipateChild,
        approveType: tour.approveType,
        firstStartDateAt: Utils.toCalendarDate(tour.firstStartDateAt),
        firstStartTimeAt: Utils.toTime(tour.firstStartTimeAt),
        regularFinishAt: tour.regularFinishAt ? Utils.toCalendarDate(tour.regularFinishAt) : null,
        regularMode: tour.regularMode,
        _regularMode: tour.regularMode === 'single' ? 'single' : 'regular',
        discount: tour.discount,
        durationDays: tour.durationDays,
        startPoints: tour.startPoints.map((v) => ({ cityId: v.city.id, timeAt: Utils.toTime(v.timeAt) })),
        route: tour.route.map((v) => ({
          cityId: v.city.id,
          dateAt: Utils.toCalendarDate(v.dateAt),
          nights: v.nights,
  ...
  const submitHandler: SubmitHandler<z.infer<typeof formScheme>> = async (vals) => {
    const action = async () => {
      ...
      const tour = await ToursService.updateTour({
        ...v,
