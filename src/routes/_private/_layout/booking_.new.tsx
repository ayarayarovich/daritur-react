            ...
            {!selectedTourId && (
              <>
                ...
                <ComboBox
                  ...
                  size='sm'
                  grow
                  items={availableTours.items}
                  onInputChange={availableTours.setFilterText}
                  inputValue={availableTours.filterText}
                  onSelectionChange={(v) => {
                    if (!v) {
                      return
                    }
                    setSelectedTourId(Number(v))
          ...
          {prepareDate.data && (
            <>
              ...
              <div className='mt-4 flex items-center gap-2'>
                ...
                <div className={boxCva({})}>
                  {new Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(
                    (prepareDate.data.tour.priceParticipateAdult + prepareDate.data.tour.priceTransferAdult) * formValues.customers.length +
                      (prepareDate.data.tour.priceParticipateChild + prepareDate.data.tour.priceTransferChild) * 0,
                  )}
                </div>
              </div>
